...
- The user wants parallel work across branches without disrupting the current workspace.
...
- **Shell commands:** set `working_directory` to the worktree's absolute path.
- **File operations (Read, Write, StrReplace, Glob, Grep):** use absolute paths pointing into the worktree.
- **Dependencies:** run install commands (`npm install`, `pip install -r requirements.txt`, etc.) inside the worktree if needed — it is a fresh checkout.

> **Limitation:** Cursor IDE features (linter integration, IntelliSense, open-file tabs) are tied to the original workspace. For large or multi-file tasks, suggest the user opens the worktree directory in a **new Cursor window** for full IDE support.
...
All git operations run with `working_directory` set to the worktree path.
...
- How to open it in a new editor window for continued work.
...
- For tasks that need linting, testing, or full IDE support, recommend the user opens the worktree in a separate Cursor window.
