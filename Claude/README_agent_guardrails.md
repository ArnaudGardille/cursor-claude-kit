...
```mermaid
flowchart TD
  A[Developer request / ticket] --> B{Type of work?}

  B -->|Product/code change| C[Implementer\n(main agent)]
  B -->|Lint/type failures| S[Skill: lint-fix\n(ruff/mypy loop)]
  B -->|Eval regression / bad agent behavior| E[Subagent: Eval Doctor\n(trace + metrics RCA)]

  C --> V[Subagent: Verifier\n(minimal diff + safety + checks)]
  S --> V

  V -->|PASS| M[Merge / ship]
  V -->|FAIL| D{What failed?}

  D -->|Tests/runtime error| G[Subagent: Debugger\n(repro → root cause → minimal fix)]
  D -->|Safety risk / destructive ops| H[Require plan:\nMigration + Rollback + Staging + Explicit confirmation]
  D -->|Missing verification| R[Run required checks:\nruff, mypy, pytest, integration]

  G --> C
  H --> C
  R --> V

  E --> P[Pick smallest general fix:\n1) Tool contract\n2) Decision logic\n3) Retrieval\n4) Prompt\n5) Architecture]
...
## What this repo enforces (the only things that matter)
...
## Contribution rules (keep this repo useful)
